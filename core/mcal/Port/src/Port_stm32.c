...
void usart_Configuration(void)
{
    ...
    NVIC_InitTypeDef NVIC_InitStructure;	//¶¨ÒåÊý¾Ý½á¹¹Ìå
    
    NVIC_SetVectorTable(NVIC_VectTab_FLASH, 0x0000);//½«ÖÐ¶ÏÊ¸Á¿·Åµ½FlashµÄ0µØÖ·

    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_1);//ÉèÖÃÓÅÏÈ¼¶ÅäÖÃµÄÄ£Ê½£¬ÏêÇéÇëÔÄ¶ÁÔ­²ÄÁÏÖÐµÄÎÄÕÂ
    ...
    NVIC_Init(&NVIC_InitStructure);	//½«½á¹¹Ìå¶ªµ½ÅäÖÃº¯Êý£¬¼´Ð´Èëµ½¶ÔÓ¦¼Ä´æÆ÷ÖÐ   
    usart_send_char('A');
}
